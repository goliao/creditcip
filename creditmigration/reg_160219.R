...
# amt of total debt in the 5 ccys that I analyze
#  single year 
### df1<-df1 %>% filter(year==2015)
amtsum_allccy<-df1 %>% select(amt) %>% sum()
df1 %>% group_by(ccy) %>% summarise(pct=sum(amt)/amtsum_allccy) %>% arrange(desc(pct))
# amt of foreign issuance in the 5 ccys that I analyze
amtsumF_allccy<-df1 %>% filter(foreign==1) %>% select(amt) %>% sum()
df1 %>% filter(foreign==1) %>%  group_by(ccy) %>% summarise(pct=sum(amt)/amtsumF_allccy) %>% arrange(desc(pct))
...
summaryBy(amt~ccy+foreign,data=df1,FUN=sum) %>% orderBy(~-foreign-amt.sum,.)
summaryBy(amt~ccy,data=df1,FUN=sum) %>% orderBy(~-amt.sum,.)


df2<-df1  %>% 
  filter(amt>=50,ytofm>=2, ytofm<99,nrating<=16, nrating>=2, (pub=="Public" | pub=="Sub."), 
         ...
         # group_by(foreign) %>% summarise(amttot=sum(amt)/1000)
  
  
# UOP
dfsuop<-df1 %>% summaryBy(amt~Euop,data=.,FUN=sum) %>% orderBy(~-amt.sum,.) %>% mutate(sumpct=amt.sum/amtsum_allccy)
View(dfsuop)
write.csv(dfsuop,'uop.csv')

  
  
  
# weighted avg duration
df2 %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))


df2 %>% filter(ccy=="EUR") %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="GBP") %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="JPY") %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="AUD") %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))

df2 %>% filter( modnat=="Eurozone") %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter( modnat=="United Kingdom") %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter( modnat=="Japan") %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter( modnat=="Australia") %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))

df2 %>% filter(ccy=="EUR", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="GBP", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="JPY", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="AUD", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))

df2 %>% filter(ccy=="EUR" | modnat=="Eurozone", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="GBP" | modnat=="United Kingdom", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="JPY" | modnat=="Japan", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter(ccy=="AUD" | modnat=="Australia", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))

df2 %>% filter( modnat=="Eurozone", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter( modnat=="United Kingdom", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter( modnat=="Japan", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))
df2 %>% filter( modnat=="Australia", foreign==1) %>% mutate(sumamt=sum(amt),weight=amt/sum(amt)) %>% 
  summarise(amt=sum(amt)/1000, nrating=sum(nrating*weight),ytofm=sum(ytofm*weight))

df2 %>% icollapse(.,"AUD","Australia") %>% View(.)
df2 %>% filter(ccy=="USD",modnat=="Australia",year==2009, month==10) %>% select(i,amt,ytofm,nrating) %>% View()

df1 %>% 
  mutate(year=(year(d))) %>% 
  filter(pub %ni% c("Govt."), mdealtype %ni% c("P","ANPX","M","EP","CEP","TM","PP","R144P"), 
         secur %ni% c("Cum Red Pfd Shs", "Non-Cum Pref Sh" , "Preferred Shs" ,"Pfd Stk,Com Stk"),
         tf_mid_desc!='Government Sponsored Enterprises', year==2015) %>% 
  group_by(foreign) %>% summarise(amttot=sum(amt)/1000)
...
df1 %>% 
  ...
  filter(pub %ni% c("Govt."), mdealtype %ni% c("P","ANPX","M","EP","CEP","TM","PP","R144P"), 
         ...
         tf_mid_desc!='Government Sponsored Enterprises') %>% 
  filter(foreign==1) %>% 
  group_by(ccy, year) %>% summarise(amttot=sum(amt)/1000) %>% 
  spread(.,ccy,amttot) %>% write.csv(.,'temp.csv')
  group_by(year) %>% summarise(foreignfraction=amttot)
...
df2<-df1  %>% 
  filter(amt>=50,ytofm>=2, ytofm<=99999,nrating<=16, (pub=="Public" | pub=="Sub."), 
         ...
         tf_mid_desc!='Government Sponsored Enterprises',
         nrating>1) %>% 
  icollapse_all(.) %>% full_join(.,priceraw,by='date')

df2 %>% filter(year>2001) %>% 
  tidyr::gather(.,'type','value',-date) %>% 
  filter(type %in% c('IN_euus','I_net_euus')) %>% 
  ggplot(data=.,aes(x=date,y=value,colour=type))+geom_line()+geom_point()

df2 %>% filter(year>2001) %>% 
  tidyr::gather(.,'type','value',-date) %>% 
  filter(type %in% c('I_net_euus')) %>% 
  ggplot(data=.,aes(x=date,y=value,colour=type))+geom_line()+geom_point()
...
df2 %>% select(I_net_euus,Cdif_euus_30,date) %>% dplyr::arrange(desc(date)) %>% View(.)
df2 %>% lm(I_net_euus~Cdif_euus_30,data=.)
...
regtemp<-function(dfreg){
  reg10_1<- dfreg %>% lm(diff(eubs10)~I_net_euus[-1],data=.)
  reg10_2<- dfreg %>% lm(diff(bpbs10)~I_net_gbus[-1],data=.)
  reg10_3<- dfreg %>% lm(diff(jybs10)~I_net_jpus[-1],data=.)
  reg10_4<- dfreg %>% lm(diff(adbs10)~I_net_auus[-1],data=.)
  reg7_3<- dfreg %>% lm(diff(jybs7)~I_net_jpus[-1],data=.)
  stargazer(reg10_1, reg10_2, reg10_3, reg7_3, reg10_4,type='text',report="vct*")
}
...
df1 %>% dplyr::tbl_df() %>% 
  filter(amt>=50,ytofm>=2, ytofm<=99999,nrating<=16, (pub=="Public" | pub=="Sub."), 
         ...
         tf_mid_desc!='Government Sponsored Enterprises',
         nrating>1) %>% 
  icollapse_all(.) %>% full_join(.,priceraw,by='date') %>% View(.)
