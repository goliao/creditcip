...
#strparams<-readr::read_lines('iss_161015.R',skip = 4,n_max=20);strparams<-strparams[1:grep(pattern="#' ---",strparams)-1];params<-yaml.load(str_c(strparams,collapse='\n') %>% str_replace_all("#'",""))
...
resirf %>% plot.irf() %>% ggsave(file=str_c(params$fileout,'active_oirf_icb.pdf'),.,width=8,height=5)
...
res1<-vars::VAR(dtvar,1)
resirf<-vars::irf(res1,ci=.95) 
resirf %>% plot.irf() %>% ggsave(file=str_c(params$fileout,'active_oirf_ibc.pdf'),.,width=8,height=5)
...
resirf %>% plot.irf() %>% ggsave(file=str_c(params$fileout,'active_oirf_partial.pdf'),.,width=8,height=5)

#+ eval=params$run.var
dtvar<-dtreg.m[date<'2016-09-01'][ccy=='eur'][,.(i_netflow,netmisp)]
res1<-vars::VAR(dtvar,1)
resirf<-vars::irf(res1,ci=.95,impulse='netmisp',response='i_netflow') 
resirf %>% plot.irf.single() %>%  ggsave(file=str_c(params$fileout,'active_oirf_inetdev.pdf',.,width=8,height=5)
...
statacmd <- str_c('clear matrix
                 ...
                 *graph export "',params$fileout,'VAR_irfeur_A.png",replace
                 * A
                 var i_netflow credit cip if strccy=="eur", lags(1)
                 ...
                 graph export "',params$fileout,'VAR_oirfeur_A.png",replace
                 ...
                 * reorder
                 ...
                 irf describe eur_oirf
                 irf cgraph  (eur_oirf credit credit oirf,`graphopt\')  (eur_oirf credit cip oirf,`graphopt\')  (eur_oirf credit i_netflow oirf,`graphopt\') (eur_oirf cip credit oirf,`graphopt\') (eur_oirf cip cip oirf,`graphopt\')  (eur_oirf cip i_netflow oirf,`graphopt\') , cols(3) graphregion(color(white)) bgcolor(white)
                 graph export "',params$fileout,'VAR_oirfeur_B.png",replace
                 ...
                 * C: lags=4 according to AIC
                 ...
                 irf describe eur_oirf
                 irf cgraph  (eur_oirf credit credit oirf,`graphopt\')  (eur_oirf credit cip oirf,`graphopt\')  (eur_oirf credit i_netflow oirf,`graphopt\') (eur_oirf cip credit oirf,`graphopt\') (eur_oirf cip cip oirf,`graphopt\')  (eur_oirf cip i_netflow oirf,`graphopt\') , cols(3) graphregion(color(white)) bgcolor(white)
                 graph export "',params$fileout,'VAR_oirfeur_4lags.png",replace
                 ...
                 * D: c-b and iss
                 ...
                 graph export "',params$fileout, 'VAR_oirfeur_netdeviss.png",replace')
...
#print('EUR IRF')
#include_graphics('',params$fileout,'VAR_irfeur_A.png') 
print('EUR OIRF order: Issuance Credit CIP')
include_graphics(str_c(params$fileout,'VAR_oirfeur_A.png') )
print('EUR OIRF order: Issuance CIP Credi')
include_graphics(str_c(params$fileout,'VAR_oirfeur_B.png') )
print('EUR OIRF 4 lags order: Issuance Credit CI')
include_graphics(str_c(params$fileout,'VAR_oirfeur_4lags.png') )
print('EUR OIRF order: Issuance Net deviation (credit-cip)')
include_graphics(str_c(params$fileout,'VAR_oirfeur_netdeviss.png'))
...
ggsave(file=str_c(params$figfile,'active_eurcreditcip.pdf'),width=9,height=6)
