...
* new "filedate" import residualized credit spread from r
	use dta/creditmisp_`filedate'.dta,clear
...
* new `filedate' import effective residualized credit spread from r
	use "dta/netmisp_`filedate'.dta",clear
	gen month=month(date)
	gen year=year(date)
	gen monthly = ym(year,month)
	format monthly %tm
...
* import misps cip long
	use dta/misplong_`filedate'.dta,clear
	gen month=month(date)
	gen year=year(date)
	gen monthly = ym(year,month)
	format monthly %tm
