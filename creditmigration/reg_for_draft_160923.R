...
# properly do IV
  ...
  eststo: xi: reg D_abs_netmisp I_both_iss i.ccy,robust 
        ...
        esttab, order(I_both*) bracket r2 star(* 0.10 ** 0.05 *** 0.01) nogaps', data.in=dt.iss.mat.m)#[ccy %in% c('eur','gbp','chf','cad')]
  
  dt.iss.mat.m[ccy=='cad',.(date,I_both.iss,I_both.mat)] %>% ggplotw()
  
  
  
  dt.iss.mat.m[,.(date,I_both.iss,I_both.mat,ccy)] %>% ggplotw
  dt.iss.mat.m %>% write.dta('temp.dta')
