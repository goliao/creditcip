...
if(quickstart==1){
  ...
  save.image(file='preprocessedtemp.RData')
}
...
if (FALSE) {
  ...
  aa [,.N,.(descr)][order(-N)] %>% head(50) %T>% dt2clip()
  
  
  aa[str_to_lower(descr) %like% 'step'][,.N,.(descr)][order(-N)]
  aa[,.N,secur %like% 'step'| str_to_lower(descr) %like% 'step']
...
#' 
#' #### EUR OIRF order: Issuance Credit CIP 
#' ![](../paper/figures/VAR_oirfeur_A.png)
#' 
#' #### EUR OIRF order: Issuance CIP Credit
#' ![](../paper/figures/VAR_oirfeur_B.png)
#' 
#' #### EUR OIRF 4 lags order: Issuance Credit CIP
#' ![](../paper/figures/VAR_oirfeur_4lags.png)
#' 
#' #### EUR OIRF order: Issuance Net deviation (credit-cip) 
#' ![](../paper/figures/VAR_oirfeur_netdeviss.png)
...
#' **Summary of all bond data**
dtl.bonds %>% table.summary()
...
dtl.bonds.global %>% table.summary()
...
#' #### reg F.issflow (next Qrt) netmisp
reg_issnetdev<-list()
for (iccy in c('eur','gbp','jpy','aud','chf','cad')){
