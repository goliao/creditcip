...
dtl[,.(.N,min(nrating),max(nrating)),rating_bucket][order(rating_bucket)]
#0 nonrated
#1 HY
#2 BBB- or better
#3 AA- or better
...
dtl[,.(.N),.(ytm<7,ytm_bucket)]

ysShort<-resyldsprdv4(dtl[ytm<7],dtm$prl,regversion=3.5,adjccybs=1,returndt=T,parallel.core.=1)

corpbasis_short=ysShort$regresult[ccy=='eur',.(date,`Short Maturity`=est)]

ysLong<-resyldsprdv4(dtl[(ytm>=7)],dtm$prl,regversion=3.5,adjccybs=1,returndt=T,parallel.core.=1)
...
dtl2[,.N,.(ccy,upnat=='United States')]
#dtl2[,.N,.(ccy,upnat=='Eurozone')]
#dtl2[,.N,upnat][order(-N)]

nonUS<-resyldsprdv4(dtl2[upnat!='United States'],dtm$prl,regversion=4,adjccybs=1,returndt=T,parallel.core.=1)
#nonUS<-resyldsprdv4(dtl2[upnat!='United States'],dtm$prl,regversion=4,adjccybs=1,returndt=T,parallel.core.=1)
corpbasis_nonUS_eur<-nonUS$regresult[ccy=='eur',.(date,`Non-U.S. issuer`=est)]

US<-resyldsprdv4(dtl2[upnat=='United States'],dtm$prl,regversion=4,adjccybs=1,returndt=T,parallel.core.=1)
#US<-resyldsprdv4(dtl2[upnat=='United States'],dtm$prl,regversion=4,adjccybs=1,returndt=T,parallel.core.=1)
corpbasis_US_eur<-US$regresult[ccy=='eur',.(date,`U.S. issuer`=est)]

corpbasis_eur=merge(corpbasis_US_eur,corpbasis_nonUS_eur,by='date')
Lcolor=c('red','dark blue')
corpbasis_eur  %>% ggplotw() +theme_few()+xlab('')+ylab('basis points')+theme(legend.position='bottom',legend.title=element_text(colour='white'))+ labs(fill='')+geom_hline(yintercept = 0,colour='grey')+ scale_color_manual(values=Lcolor)
ggsave(file='corpbasis_USnonUS.pdf',width=4, height=4,units='in')
...
############################
############################
# TREASURY VS CORP BASIS (AVG of 6 CCYs; HIGH GRADE/Short MATURITY)
###
...
load('data_treasury_corp_basis.RData')
...
#### ### GRAPH
dtA$factors <- factor(dtA$variable, levels = c("1-3Y Corp","1-7Y Corp","Treasury"))
main_theme = list(theme_gray(base_size = 16),
                  theme(axis.text.x = element_text(angle = 0, hjust = .5),
                        ...
                        legend.margin=margin(c(-8,5,0,5))))
...
plot1 <- ggplot() +
  ...
  main_theme +
  scale_color_manual(labels=c("1-3Y Corp","1-7Y Corp","Treasury", "CIP Deviation"), breaks = c("1-3Y Corp","1-7Y Corp","Treasury", "CIP Deviation"),
                     values=c("blue","dark green","red", "gray")) +
  theme(legend.position='right',legend.title=element_text(colour='white')) +
  guides(color = guide_legend(order = 1), fill = guide_legend(order = 2))

plot1
ggsave(filename = 'treasurycorp2.pdf',width=7.5, height=5,units='in')

box(which = "plt1", lty = "solid")
