...
figurespath='../../figures/'
source('../util.r')
creditcip.result<-plot.panel.creditcip(dtm$prw,ys1m$regresult,filename=str_c('fig_panel_usd.pdf'),yrstr.='5',wide=T) 


# Without Narrative
Labels=c('CIP deviation (5 yr)','Residualized credit spread differential (EU-US)'); Ltype=c('solid','dashed'); Lcolor=c('red','darkblue')
creditcip.result[[2]][ccy=='eur'] %>% ggplot(data=.,aes(x=date,y=value))+geom_line(aes(linetype=variable,colour=variable))+geom_errorbar(aes(ymin=cimin,ymax=cimax),colour='lightgrey',alpha=.5) +xlab('')+ylab('basis points')+scale_color_manual('',labels =Labels,values=Lcolor)+scale_linetype_manual('',labels =Labels,values=Ltype)+theme_few()+scale_x_date(breaks=scales::pretty_breaks(n=6))+theme(legend.position='bottom')#+annotate('text',x=ymd('2015-06-01'),y=10,label=str_c('cor=',as.character(round(creditcip.result[[3]][ccy=='eur',corr],2))))#+ theme(text = element_text(size=12))+ theme(axis.title=element_text(face="bold",size=12),axis.text=element_text(size=12,face="bold")) 
...
dtreg.q[ccy=='eur' & date<ymd('2017-01-01'),.(date,i_netflow,netmisp,netcimax,netcimin)] %>% ggplot(aes(x=date,y=netmisp)) +geom_bar(aes(x=date,y=i_netflow*secaxfactor,fill='Issuance flow (EU to US)'),stat="identity")+geom_line(aes(x=date,y=netmisp,color='Net deviation'),size=1.1)+scale_y_continuous(sec.axis = sec_axis(~./secaxfactor,name='Issuance flow as % of total issuance'))+theme_few()+ylab('Net deviation in basis points')+geom_errorbar(aes(ymin=netcimin,ymax=netcimax),colour='red',alpha=.2)+theme(legend.position='bottom')+labs(x='')+scale_colour_manual(" ", values=c("Net deviation" = "red", "Issuance flow (EU to US)" = "darkblue"))+
...
aa[,ccy:=toupper(ccy)]
ggplot(aa,aes(credit,cip,colour=ccy))+geom_point(aes(colour=ccy)) +scale_color_discrete(guide = guide_legend(title = ""))+ geom_abline(intercept=aa.reg[[1]],slope=aa.reg[[2]])+xlab('Residualized Credit Spread Differential in basis points')+ylab('CIP deviation (5yr) in basis points')+geom_hline(yintercept=0,colour='grey')+geom_vline(xintercept=0,colour='grey')+scale_x_continuous(breaks=scales::pretty_breaks(n=13))+scale_y_continuous(breaks=scales::pretty_breaks(n=7))+theme_few()+annotate('text',x=-80,y=40,label=str_c('cor=',as.character(round(aa[,cor(credit,cip)],3))))+annotate('text',x=0,y=-90,label='Costly to \nswap to USD')+annotate('text',x=-125,y=0,label='Lower local \n credit spread')
