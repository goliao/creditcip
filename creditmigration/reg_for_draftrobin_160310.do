...
tsline i_net_USDEUR if year>2005 & year<2009, recast(sc) mc(green) legend(label(1 "Pre-2009")) || tsline i_net_USDEUR if year>=2009 & year<2014, recast(sc) mc(red) msymbol(+) legend(label(2 "2009-2013: FED QE")) || tsline i_net_USDEUR if year>=2014 & year<2016, recast(sc) mc(blue) legend(label(3 "Post-2014: ECB QE")) ytitle("iss_EUR->USD")
 ...
 * Table 6: All currencies: basis
    ...
    * capture rm `filename'.csv
    * eststo clear
    * eststo: reg D.eubs10 i_net_USDEUR post07 D.eubs1 if abs(I_net_USDEUR)>0
    * eststo: reg D.eubs10 i_USDeu post07 D.eubs1 if abs(I_net_USDEUR)>0
    * eststo: reg D.eubs10 i_EURus post07 D.eubs1 if abs(I_net_USDEUR)>0

    * eststo: reg D.bpbs10 i_net_USDGBP post07 D.bpbs1 if abs(I_net_USDGBP)>0
    * eststo: reg D.bpbs10 i_USDgb post07 D.bpbs1 if abs(I_net_USDGBP)>0
    * eststo: reg D.bpbs10 i_GBPus post07 D.bpbs1 if abs(I_net_USDGBP)>0

    * eststo: reg D.jybs10 i_net_USDJPY post07 D.jybs1 if abs(I_net_USDJPY)>0
    * eststo: reg D.jybs10 i_USDjp post07 D.jybs1 if abs(I_net_USDJPY)>0
    * eststo: reg D.jybs10 i_JPYus post07 D.jybs1 if abs(I_net_USDJPY)>0

    * eststo: reg D.adbs10 i_net_USDAUD post07 D.adbs1 if abs(I_net_USDAUD)>0
    * eststo: reg D.adbs10 i_USDau post07 D.adbs1 if abs(I_net_USDAUD)>0
    * eststo: reg D.adbs10 i_AUDus post07 D.adbs1 if abs(I_net_USDAUD)>0
...
graph twoway (tsline F.i_net_USDEUR_6mf) (tsline Cdif_euus_30_eff,yaxis(2)) if year>=2002 & year<2016, legend(label(1 "IssEUtoUS6m/TotIss") lab(2 "Corp A Credit Sprd Diff EUR-USD")) ttitle(none) ytitle("Issuance in billion dollars") ytitle("basis points",axis(2))
graph export "../paper/figures/eurissuancecredit.eps",replace

* * generate graph of issuance and basis
graph twoway (tsline I_net_USDEUR if year>=2006 & monthly<=d(1oct2015))  (tsline D.eubs10 if year>=2006 & monthly<=d(1oct2015),yaxis(2)) if year>=2006 & year<2016, legend(label(1 "Issuance_EU->US") lab(2 "Change in EUR-USD cross-currency basis")) ttitle("") ytitle("Issuance in billion dollars") ytitle("monthly change in currency basis",axis(2))
