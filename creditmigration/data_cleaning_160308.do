...
insheet using "/Users/gliao/Dropbox/Research/ccy basis/data/FoF/BDLeverage.csv", clear 
gen quarterly=yq(year,quarter)
format quarterly %tq
drop if year<1965 | (year==1965 & quarter<3)
gen bdleverage=bdasset/(bdasset-bdliability)
gen bdreponet=bdliability-bdrepoasset
gen bdasset2=bdasset-bdrepoasset
gen bdliability2=bdliability-bdrepoasset
gen bdleverage2=bdasset2/(bdasset2-bdliability2)
* gen bdleverage3=(bdasset-bdrepoasset)/(bdasset-bdliability)
tsset quarterly
sort quarterly
keep year quarter quarterly bdleverage*


gen lnlev2=log(bdleverage2)
gen lnlev=log(bdleverage)

egen lnlev2mean=mean(lnlev2)
egen lnlev2sd=sd(lnlev2)
gen lnlevnorm2=(lnlev2-lnlev2mean)/(lnlev2sd)

gen lnlev2D=D.lnlev2
gen lnlevD=D.lnlev

egen lnlev2Dmean=mean(lnlev2D)
egen lnlev2Dsd=sd(lnlev2D)
gen levfac2=(lnlev2D-lnlev2Dmean)/lnlev2Dsd

* tsline lnlevnorm levfac2
gen dq1=1 if quarter==1
gen dq2=1 if quarter==2
gen dq3=1 if quarter==3
gen dq4=1 if quarter==4
replace dq1=0 if dq1==.
replace dq2=0 if dq2==.
replace dq3=0 if dq3==.
replace dq4=0 if dq4==.

reg levfac2 dq2 dq3 dq4
predict levfac2adj,resid
...
format monthly %tm
* tsline levfac2 levfac2adj
* su lnlevnorm2 levfac2
save bdlev.dta,replace




* clean baml bond indices
use baml_indices_160207.dta,clear
...
sort monthly
    order monthly, first
    order year, last
    collapse (lastnm) comgbp5y-year, by(monthly) fast
tsset monthly
drop if year<1995
...
* citi
cd "/Users/gliao/Dropbox/Research/ccy basis/data/citi"
insheet using citioas.csv, clear name
rename ba Cdif_euus_citi
rename ed Cdif_euus_citi_liboroas
rename date datestr
gen date=date(datestr,"MDY",2050)
format date %td
gen month=month(date)
...
sort monthly
tsset monthly
keep monthly date  Cdif*
cd "/Users/gliao/Dropbox/Research/ccy basis/creditmigration/"
save citioas.dta,replace
...
merge 1:1 monthly using citioas.dta
