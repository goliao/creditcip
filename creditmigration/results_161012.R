...
#+ eval=getOption('gl')$run.var
  ...
  resirf %>% plot.irf() %>% ggsave(file=str_c(getOption('gl')$fileout,'active_oirf_icb.pdf'),.,width=8,height=5)
...
#+ eval=getOption('gl')$run.var
  ...
  res1<-vars::VAR(dtvar,1)
  resirf<-vars::irf(res1,ci=.95) 
  resirf %>% plot.irf() %>% ggsave(file=str_c(getOption('gl')$fileout,'active_oirf_ibc.pdf'),.,width=8,height=5)
...
## A matrix
  ...
  resirf %>% plot.irf() %>% ggsave(file=str_c(getOption('gl')$fileout,'active_oirf_partial.pdf'),.,width=8,height=5)

#+ eval=getOption('gl')$run.var
  dtvar<-dtreg.m[date<'2016-09-01'][ccy=='eur'][,.(i_netflow,netmisp)]
  res1<-vars::VAR(dtvar,1)
  resirf<-vars::irf(res1,ci=.95,impulse='netmisp',response='i_netflow') 
  resirf %>% plot.irf.single() %>%  ggsave(file=str_c(getOption('gl')$fileout,'active_oirf_inetdev.pdf',.,width=8,height=5)
...
#+ statacalc,include=FALSE,eval=(getOption('gl')$run.var & getOption('gl')$run.stata)
  statacmd <- str_c('clear matrix
      ...
      *graph export "',getOption('gl')$fileout,'VAR_irfeur_A.png",replace
      * A
      var i_netflow credit cip if strccy=="eur", lags(1)
      ...
      graph export "',getOption('gl')$fileout,'VAR_oirfeur_A.png",replace
      ...
      * reorder
      ...
      irf describe eur_oirf
      irf cgraph  (eur_oirf credit credit oirf,`graphopt\')  (eur_oirf credit cip oirf,`graphopt\')  (eur_oirf credit i_netflow oirf,`graphopt\') (eur_oirf cip credit oirf,`graphopt\') (eur_oirf cip cip oirf,`graphopt\')  (eur_oirf cip i_netflow oirf,`graphopt\') , cols(3) graphregion(color(white)) bgcolor(white)
      graph export "',getOption('gl')$fileout,'VAR_oirfeur_B.png",replace
      ...
      * C: lags=4 according to AIC
      ...
      irf describe eur_oirf
      irf cgraph  (eur_oirf credit credit oirf,`graphopt\')  (eur_oirf credit cip oirf,`graphopt\')  (eur_oirf credit i_netflow oirf,`graphopt\') (eur_oirf cip credit oirf,`graphopt\') (eur_oirf cip cip oirf,`graphopt\')  (eur_oirf cip i_netflow oirf,`graphopt\') , cols(3) graphregion(color(white)) bgcolor(white)
      graph export "',getOption('gl')$fileout,'VAR_oirfeur_4lags.png",replace
      ...
      * D: c-b and iss
      ...
      graph export "',getOption('gl')$fileout, 'VAR_oirfeur_netdeviss.png",replace')
...
#print('EUR IRF')
#include_graphics('',getOption('gl')$fileout,'VAR_irfeur_A.png') 
print('EUR OIRF order: Issuance Credit CIP')
include_graphics(str_c(getOption('gl')$fileout,'VAR_oirfeur_A.png') )
print('EUR OIRF order: Issuance CIP Credi')
include_graphics(str_c(getOption('gl')$fileout,'VAR_oirfeur_B.png') )
print('EUR OIRF 4 lags order: Issuance Credit CI')
include_graphics(str_c(getOption('gl')$fileout,'VAR_oirfeur_4lags.png') )
print('EUR OIRF order: Issuance Net deviation (credit-cip)')
include_graphics(str_c(getOption('gl')$fileout,'VAR_oirfeur_netdeviss.png'))
...
ggsave(file=str_c(getOption('gl')$figfile,'active_credit.pdf'),width=9,height=6)
...
# ggsave(file=str_c(getOption('gl')$figfile,'fig1_cip_160830.pdf'),width=9,height=6)
#' 1y cip
#+ eval=getOption('gl')$show.credit.cip
...
# ggsave(file=str_c(getOption('gl')$figfile,'fig2_cip_1y_160929.pdf'),width=9,height=6)

#' Graphing CIP and credit mispriing overlay
#+ eval=getOption('gl')$show.credit.cip, fig.width=8
creditcip.result<-plot.panel.creditcip(dtm$prw,ys1m$regresult,filename=str_c(getOption('gl')$figfile,'active_panel_usd.pdf'),yrstr.='5',wide=T) 
...
ggsave(file=str_c(getOption('gl')$figfile,'active_creditcipscatter.pdf'),width=9,height=6)

#' Net deviation
#+ eval=getOption('gl')$show.credit.cip
ys1meff$regresult %>% plot.netdev(fileout=str_c(getOption('gl')$figfile,'active_netdev.pdf'))
...
ggsave(file=str_c(getOption('gl')$figfile,'active_eurcreditcip.pdf'),width=9,height=6)
...
#+ eval=getOption('gl')$show.credit.cip
  ...
  ggsave(file=str_c(getOption('gl')$figfile,'active_hghy_eur.pdf'),width=9,height=6)
...
#+ eval=getOption('gl')$show.credit.cip, warning=F
  ...
  ggsave(file=str_c(getOption('gl')$figfile,'active_HGLG_all.pdf'),width=10.5,height=6.5)
...
zz<-plot.panel.creditcip.any.ccy(ys.eur,ys.eur.eff,wide=T,filename=str_c(getOption('gl')$figfile,'active_panel_eur.pdf'))
...
zz<-plot.panel.creditcip.any.ccy(ys.gbp,ys.gbp.eff,wide=T,filename=str_c(getOption('gl')$figfile,'active_panel_gbp.pdf'))
#' Without USD bonds in the calculation
#+ eval=F
...
if (FALSE) {
  ...
  aa[,.N,str_to_lower(securityType)]
  sdc[,.N,rule144a]
  sdc[,.N,glaw]
  #' amt
